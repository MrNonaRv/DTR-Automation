...
const regex = /ws\.eachRow\(\(row, rowNumber\) => \{\s*const noVal = String\(row\.getCell\(1\)\.value \|\| ''\)\.trim\(\);\s*\/\/ Skip header\s*if \(rowNumber === 1 && noVal\.toLowerCase\(\) === 'no\.'\) return;\s*const name = String\(row\.getCell\(2\)\.value \|\| ''\);\s*const dept = String\(row\.getCell\(3\)\.value \|\| ''\);\s*newPeople\.push\(\{ id: Math\.random\(\)\.toString\(36\)\.slice\(2, 10\), empNo: noVal \|\| String\(rowNumber - 1\), name: name\.trim\(\), dept: dept\.trim\(\) \}\);\s*\}\);/m;
...
code = code.replace(regex, newImportLogic);
fs.writeFileSync('src/components/ScannerTool.tsx', code);
console.log("Patched import logic again!");
