...
export async function generateDTR(employeeName, period, records, printRange = 'full') {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.on("error", reject);
            const drawDTR = (startX, startY, width) => {
                let y = startY;
                const leftX = startX;
...
export async function generateAllDTRs(period, employees, printRange = 'full') {
    return new Promise((resolve, reject) => {
        try {
            const doc = new PDFDocument({ size: "A4", margin: 20 });
            const buffers = [];
            ...
            doc.on("end", () => resolve(Buffer.concat(buffers)));
            doc.on("error", reject);
            const drawDTR = (startX, startY, width, employeeName, records) => {
                let y = startY;
                const leftX = startX;
                const rightX = startX + width;
                const logoPath = path.join(process.cwd(), "Systemlogo.jpg");
